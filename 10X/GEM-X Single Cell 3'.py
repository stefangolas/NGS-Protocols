...
class TenXGEXLibraryPrepProtocol(Protocol):
    ...
    def _setup_deck_layout(self):
        ...
        self.MIDI_CPAC = layout_item(self.lmgr, Plate96, 'MIDI_CPAC02')
        self.MIDI_OnMagnet = layout_item(self.lmgr, Plate96, 'MIDI_OnMagnet')
        self.HHS5_MIDI = layout_item(self.lmgr, Plate96, 'HHS5_MIDI')


        # Reagent containers
        self.CAR_VIALS_SMALL = layout_item(self.lmgr, ReagentTrackedEppiCarrier32, 'SMP_CAR_24_15x75_A00_0001')
    ...
    def _prepare_fragmentation_mix(self, ham_int):
        ...
        multi_dispense(ham_int, tips=self.tracked_tips_300uL, source_positions=self.buffer_eb_positions, dispense_positions=mix_position,
                       volumes=[buffer_eb_vol]*self.num_samples,
        ...
        multi_dispense(ham_int, tips=self.tracked_tips_300uL, source_positions=self.fragmentation_buffer_positions, dispense_positions=mix_position,
                       volumes=[frag_buffer_vol]*self.num_samples,
                       liquid_class='StandardVolumeFilter_Water_DispenseJet_Empty')
        ...
        multi_dispense(ham_int, self.tracked_tips_300uL, self.fragmentation_enzyme_positions, mix_position,
                       volumes=[frag_enzyme_vol]*self.num_samples,
                       liquid_class='StandardVolumeFilter_Water_DispenseJet_Empty')

        # Mix the prepared fragmentation mix
        total_vol = buffer_eb_vol + frag_buffer_vol + frag_enzyme_vol
        mix_plate(ham_int, tips = self.tracked_tips_300uL, tip_support=self.tip_support,
                num_samples = self.num_samples, plate=self.HSP_Plate, 
                mixing_volume=min(total_vol, 1000), mix_cycles=15,
                liquid_class='StandardVolumeFilter_Water_DispenseJet_Empty')
    ...
    def fragmentation_end_repair_atailing(self):
        ...
        with HamiltonInterface(simulating=self.simulation, windowed=self.windowed, server_mode=False, persistent=self.persistent) as ham_int:
            ...
            odtc_execute_protocol(ham_int, device_id=1, method_name='10x_fragmentation_protocol.xml', simulating=self.device_simulation)
    ...
    def adapter_ligation(self):
        ...
        with HamiltonInterface(simulating=self.simulation, windowed=self.windowed, server_mode=False, persistent=self.persistent) as ham_int:
            ...
            odtc_execute_protocol(ham_int, device_id=1, method_name='10x_adapter_ligation_protocol.xml', simulating=self.device_simulation)
    ...
    def sample_index_pcr(self):
        ...
        with HamiltonInterface(simulating=self.simulation, windowed=self.windowed, server_mode=False, persistent=self.persistent) as ham_int:
            ...
            odtc_execute_protocol(ham_int, device_id=1, method_name='10x_sample_index_pcr.xml', simulating=self.device_simulation)
...
if __name__ == "__main__":
    ...
    # protocol.initialize()
    # protocol.fragmentation_end_repair_atailing()
    # protocol.post_fragmentation_spriselect()
    # protocol.adapter_ligation()
    # protocol.post_ligation_cleanup()
    # protocol.sample_index_pcr()
    # protocol.final_size_selection()
