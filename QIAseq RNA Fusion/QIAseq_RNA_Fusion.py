...
class QIAseqRNAFusionProtocol(Protocol):
    ...
    def _setup_deck_layout(self):
        ...
        self.HSP_CPAC = layout_item(self.lmgr, ReagentTrackedPlate96, 'HSP_CPAC')
    ...
    def initialize(self):
        ...
        with HamiltonInterface(simulating=self.simulation, windowed=self.windowed, server_mode=False, persistent=self.persistent) as ham_int:
            ...
            hhs_set_simulation(ham_int, 1 if self.simulation else 0)
    ...
    def sample_cleanup_1(self):
        ...
        with HamiltonInterface(simulating=self.simulation, windowed=self.windowed, server_mode=False, persistent=self.persistent) as ham_int:
            ...
            transport_resource(ham_int, self.MIDI_OnMagnet, self.HHS5_MIDI,
                             resource_type=GrippedResource.MIDI, core_gripper=True)
            ...
            transport_resource(ham_int, self.MIDI_OnMagnet.layout_name(), self.HHS5_MIDI.layout_name(),
                             resource_type=GrippedResource.MIDI, core_gripper=True)
            ...
            transport_resource(ham_int, self.HHS5_MIDI, self.MIDI_OnMagnet,
                             resource_type=GrippedResource.MIDI, core_gripper=True)
